...
import { BookOpen, Edit3, MessageSquare, FileText, Users, Zap, Languages } from "lucide-react";
...
const Index: React.FC = () => {
  ...
  const handleTranslationQuiz = () => navigate("/translation-quiz");
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background px-4">
      ...
      <div className="max-w-lg w-full bg-white dark:bg-card text-foreground shadow-lg rounded-lg p-8">
        ...
        <div className="space-y-4">
          ...
          <Button 
            onClick={handleTranslationQuiz} 
            variant="secondary" 
            size="lg" 
            className="w-full h-16 text-lg"
          >
            <Languages className="mr-3 h-6 w-6" />
            {t('translationPractice')}
          </Button>
        </div>
      </div>
      ...
      <div className="mt-6 text-sm text-muted-foreground text-center max-w-lg">
        ...
        <p className="mt-2">{t('translationExplanation')}</p>
      </div>
    </div>
  );
};
